...
# import evaluate
#
# class OurBilevelMinimaxTrainer(Trainer):
#
#     # ZO-Bench added: new parameters to our traininer
#     def __init__(self, model_s, evaluate_func, dev_dataset, dev_samples, eval_samples, lower_level_training_args, *args, **kwargs):
...
#
#
#                 # Bilevel added: train lower level model
...
#                             inputs_p = next(eval_iterator)
#                             tr_loss_step = self.bilevel_outer_p_step(model, inputs, inputs_p)
#                     elif args.q == 1:
...
#                     elif args.trainer == "bilevel_minimax_trainer":
#                         # update p
#                         self.gradient_update(model)
#                         # update theta
#                         _ = self.bilevel_outer_theta_step(model, self.model_s, inputs, inputs_p)
#
#                     elif args.trainer == "forward_grad":
...
#
#
#     def compute_upper_loss(self, model, model_s, inputs, inputs_lower):
#         upper_loss = self.compute_loss(model,inputs) + self.Lambda*(self.compute_loss(model, inputs_lower) - self.compute_loss(model_s, inputs_lower))
#         return upper_loss
#
#     ############## MeZO ##############
#
...
#         """
#         Perturb the parameters theta with random vector z.
#         Input:
...
#
#         for _, param in self.named_parameters_to_optim_full:
#             z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
#             param.data = param.data + scaling_factor * z * self.args.zo_eps
#
#     def zo_forward(self, model, inputs, model_s=None,  inputs_p=None):
#         """
#         Get (no gradient) upper loss from the model. Dropout is turned off too.
#         """
#         model.eval()
...
#                 if self.args.trainer=="bilevel_minimax_trainer":
#                     loss = self.compute_upper_loss(model, model_s, inputs, inputs_p)
#                 else:
...
#
#     def bilevel_outer_p_step(self, model, model_s, inputs, inputs_p):
#         model.train()
...
#
#     # @torch.no_grad()
#     def bilevel_outer_theta_step(self, model, model_s, inputs, inputs_p):
#         """
...
#         loss1 = self.zo_forward(model, model_s, inputs, inputs_p)
#
...
#             self.zo_perturb_parameters(scaling_factor=-1)
#             loss2 = self.zo_forward(model, model_s, inputs, inputs_p)
...
#             # update theta
#             for name, param in self.named_parameters_to_optim_full:
...
#         return loss1
#
#     def lower_level_train(self, model):
#         # set prameters of virtural prompt to optimize
#         for name, param in model.named_parameters():
#             if "prompt_encoder" in name:
#                 param.requires_grad = True
...
#         print(
#             f"  Number of trainable parameters in the lower level = {sum(p.numel() for p in model.parameters() if p.requires_grad)}"
#         )
#
